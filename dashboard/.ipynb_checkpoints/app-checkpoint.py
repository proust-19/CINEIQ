...
st.title("🎬 CineIQ")
st.caption("Hybrid Movie Recommendation Engine — Content + Collaborative + Sentiment")
...
if st.sidebar.button("Get Recommendations"):
    ...
    if res.status_code == 200:
        ...
        if "error" in data:
            ...
            # Main table
            st.dataframe(
                df[['title', 'genres', 'final_score']].rename(columns={
                    'title': 'Movie',
                    'genres': 'Genres',
                    'final_score': 'Score'
                }),
            ...
            fig2 = px.pie(genre_df, values='Count', names='Genre', title='Genre Distribution')
            st.plotly_chart(fig2, use_container_width=True)
    else:
        st.error("API error — make sure FastAPI is running on port 8000")

st.divider()
st.subheader("Find Similar Movies")
similar_title = st.text_input("Movie title", value="Pulp Fiction (1994)")
if st.button("Find Similar"):
    res = requests.post(f"{API_URL}/similar", json={
        "movie_title": similar_title, "n": 8
    })
    if res.status_code == 200:
        data = res.json()
        if "error" in data:
